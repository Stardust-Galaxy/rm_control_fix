...
ImuFilterBase::ImuFilterBase(const rclcpp::NodeOptions& options) : Node("imu_filter", options) {
...
bool ImuFilterBase::init(XmlRpc::XmlRpcValue& imu_data, const std::string& name) {
    ...
    initFilter(imu_data);
    imu_data_pub_ = this->create_publisher<sensor_msgs::msg::Imu>(name + "/imu_data", 100);
    imu_temp_pub_ = this->create_publisher<sensor_msgs::msg::Temperature>(name + "/imu_temp", 100);
