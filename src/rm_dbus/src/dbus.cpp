...
#define termios asmtermios // avoid conflict with termios.h
...
extern "C" {
    extern int ioctl(int fd, unsigned long request, ...) throw();
};
...
void DBus::init(const char* serial) {
    ...
    if(fd == -1) {
        RCLCPP_ERROR(rclcpp::get_logger("rm_dbus"), "Failed to open serial port");
    }
...
void DBus::getData(rm_msgs::msg::DbusData &d_bus_data) const {
    if (success_)
    {
        ...
        //TODO: move time updates to node.cpp
    }
}