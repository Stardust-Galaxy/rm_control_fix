 ...
 */
    SocketCAN::SocketCAN(rclcpp::Clock::SharedPtr clock_, rclcpp::Logger logger_) : logger_(logger_), clock_(clock_){}
    SocketCAN::~SocketCAN()
    {
        if (this->is_open())
            this->close();
                         ...
                         int thread_priority)
    {
        ...
        if (sock_fd_ == -1)
        {
            RCLCPP_ERROR(logger_,"Unable to create a CAN socket!");
            return false;
        }
        ...
        return start_receiver_thread(thread_priority);
    }
    ...
    void SocketCAN::write(can_frame* frame) const
    {
        if (!is_open())
        {
            RCLCPP_ERROR_THROTTLE(logger_, *clock_, 5., "Unable to write: Socket %s not open", interface_request_.ifr_name);
            return;
        }
        if (::write(sock_fd_, frame, sizeof(can_frame)) == -1)
            RCLCPP_DEBUG_THROTTLE(logger_, *clock_, 5., "Unable to write: The %s tx buffer may be full", interface_request_.ifr_name);
    }
    ...
    static void* socketcan_receiver_thread(void* argv)
    {
        ...
        while (!sock->terminate_receiver_thread_)
        {
            ...
            if (select(maxfd + 1, &descriptors, nullptr, nullptr, &timeout))
            {
                ...
                if (len < 0)
                    continue;
                if (sock->reception_handler_ != nullptr)
                    sock->reception_handler_(rx_frame);
            }
        }
    ...
    bool SocketCAN::start_receiver_thread(int thread_priority)
    {
